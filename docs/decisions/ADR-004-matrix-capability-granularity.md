...
- **提示词引导落地（t000034）**：粒度口径进入三处提示词（见「未覆盖」表首行），使「大盘口语 → 聚合码」由文档口径变为模型侧默认行为；提示词属 AI 变更，走评测门禁（RQ-712 三集 AND 100%）后合入。
...
| 项 | 处理 |
|---|---|
| 模型提示词引导（`大盘/整体/汇总/概况` → `query_metric`/`query_health`） | **已实现（t000034）**：三处提示词补「查询粒度」引导块 + 示例——`cohere-adapter.js` / `agens-adapter.js`（生产入口经 `openaiCompat` 用 agens 提示词）/ `gen-redteam-weekly.js` 的分类验证提示词（口径必须与线上同源，否则召回口径漂移）；锚定 `prompt-guidance.test.js` PG1–PG4。公开口语集补 6 条管理者大盘样本（`spoken-public-v2`，SP-051..056）；真模型实测 6/6 落聚合码（大盘/概况/汇总→`query_metric`，健康/告警概况/稳不稳→`query_health`，具体服务→`query_status` 对照）；门禁三集 AND 100%。 |
