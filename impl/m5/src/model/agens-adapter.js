...
const SYSTEM_PROMPT = [
  ...
  '- 整体健康 / 告警概况 / 健康报告 / 最近系统稳不稳 → query_health（健康概况）',
  '- 仅当明确指向**某个具体服务 / 资产 / 进程**的状态时才用 query_status（单资产明细）',
  'egress 示例：',
  ...
  '查询示例：',
  '- "看下整体大盘" → {"actionClass":"read","capability":"query_metric"}',
  '- "整体概况怎么样" → {"actionClass":"read","capability":"query_metric"}',
  ...
  '- 未提到的参数不要输出。',
  '只输出一个 JSON 对象，格式：',
  '{"actionClass": "read|write|egress|authorize", "capability": "query_status|query_health|query_metric|query_log|restart|clean|scale|config_change|env_switch|egress_send|egress_download|egress_mail|cred_lend", "confidence": 0.0-1.0, "subject": "执行目标资产ID（确实无目标时才为null）", "params": {"service|path|replicas": "从原话抽取"}}',
  '不要输出其他文字。',
].join('\n');
