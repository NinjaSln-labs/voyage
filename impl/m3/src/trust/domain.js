...
function approvalSnapshot(a) {
  ...
  return deepFreeze({ id: a.id, operatorId: a.operatorId, target: a.target, highRiskType: a.highRiskType, status: a.status, deadline: a.deadline.toISOString(), terminalSeq: a.terminalSeq, rejectedBy: a.rejectedBy || null, votes: a.votes.map(v => ({ personId: v.personId, webAuthnConfirmed: v.webAuthnConfirmed, seq: v.seq })) }); // 审计证据：webAuthnConfirmed（74 波）/terminalSeq（83 波）/rejectedBy（95 波）
}
