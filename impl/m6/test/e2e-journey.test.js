...
/** identity 桩（ADR-003 矩阵前置校验）：全放行——旅程用例聚焦分流语义，矩阵面另有专用用例 */
const identityAllowAll = { findById: () => ({ active: true, hasCapability: () => true }) };

test('J1 游客查询 → OK + 审计留痕', () => {
  const a = makeAudit();
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('query', 'query_status'), trustPort: makeTrust(), execPort: makeExec(), auditPort: a });
  const r = svc.handle({ actorId: 'v1', from: 'app', intent: '状态' });
  assert.strictEqual(r.status, 'OK'); assert.strictEqual(a.chain.length, 1);
});
test('J2 游客尝试重启 → 拒绝', () => {
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('execute', 'restart'), trustPort: makeTrust('rejected', null, false, 'capability_not_in_whitelist'), execPort: makeExec(), auditPort: makeAudit() });
  assert.strictEqual(svc.handle({ actorId: 'v1', from: 'app', intent: '重启' }).status, 'REJECTED');
});
test('J3 开发者重启（白名单）→ 自动Grant + exec启动', () => {
  const e = makeExec();
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('execute', 'restart'), trustPort: makeTrust('auto_granted'), execPort: e, auditPort: makeAudit() });
  const r = svc.handle({ actorId: 'dev1', from: 'cli', intent: '重启 srv1' });
  assert.strictEqual(r.status, 'OK'); assert.ok(e.started.length >= 1);
});
test('J4 开发者高危操作 → 需审批', () => {
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('execute', 'clean'), trustPort: makeTrust('pending_approval'), execPort: makeExec(), auditPort: makeAudit() });
...
test('J5 SRE 审批批准 → Outbox入队', () => {
  ...
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv(), trustPort: makeTrust(), execPort: makeExec(), auditPort: makeAudit(), outbox: new OutboxJournal({ repo }) });
...
test('J6 SRE 否决 → REJECTED', () => {
  ...
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv(), trustPort: t, execPort: makeExec(), auditPort: makeAudit() });
  assert.strictEqual(svc.resolveApproval({ approval: { id: 'ap-2' }, rejectBy: 'sre1' }).status, 'REJECTED');
});
test('J7 管理员大盘查询 → OK', () => {
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('query', 'query_metric'), trustPort: makeTrust(), execPort: makeExec(), auditPort: makeAudit() });
  assert.strictEqual(svc.handle({ actorId: 'admin1', from: 'dash', intent: '大盘' }).status, 'OK');
});
test('J8 管理员双人授权 → approved', () => {
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv(), trustPort: makeTrust(), execPort: makeExec(), auditPort: makeAudit() });
  assert.strictEqual(svc.resolveApproval({ approval: { id: 'ap-sub', status: 'pending' }, votes: ['admin1', 'sre2'] }).status, 'approved');
});
test('J9 幂等防重放', () => {
  const a = makeAudit();
  const svc = new IntegrationService({ identityPort: identityAllowAll, convPort: makeConv('query', 'query_status', null, 'idem-j9'), trustPort: makeTrust(), execPort: makeExec(), auditPort: a });
