...
- **原因**：M0-T 选型记录 + M5 整合审计 + M6 内测上线走查全部完成（全量 248/248 绿）→ 交接到 **真实部署阶段**
...
| M5 整合 + 审计 | ✅ `impl/m5` · 47 测试（Outbox + 五步串联 + 审计链 + INV-U2/U4/U5 补全） |
| M6 内测上线走查 | ✅ `impl/m6` · 21 测试（评测门禁 + 四角色走查 + 适配器契约定型） |
| 真实部署 | ⬜ 未开始 |
